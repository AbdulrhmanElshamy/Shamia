...
namespace Shamia.API.Services
{
    ...
    public class OrderService : IOrderService
    {
        ...
        public OrderService(ShamiaDbContext context, IFatoorahService fatoorahService)
        {
            _context = context;
        ...
        private static readonly Expression<Func<Order, GetOrderDataResponse>> GetAllOrdersSelector = order => new GetOrderDataResponse
        {
            Id = order.Id,
            Email = order.User.Email,
            User_Name = order.User.UserName!,
            ...
            Order_Date = order.OrderDate,
            Total_Amount = order.OrdersDetails.Sum(od => od.Offer ?? od.Unit_Price * od.Quantity_Of_Unit),
            Total_Amount_With_Discount = order.Total_Amount_With_Discount ?? order.Total_Amount, 
            Payment_Method = order.Payment_Method,
            OrderDetails = order.OrdersDetails.Select(od => new OrderDetailsResponse
            {
                Product_Name_Ar = od.Product.Name_Ar,
        ...
        private static (string Ar, string En) GetPaymentStatus(string paymentMethod)
        {
            return paymentMethod?.ToLower() switch
            {
                ...
                _ => ("غير مدفوع", "unpaid") // Default for unexpected values
            };
        }
        ...
        public async Task<(bool isSuccess, ErrorResponse? errorMessage, GetOrderDataResponse? result)> GetOrderByIdAsync(string id)
        {
            ...
            var orderDto = OrderSelector.Compile()(order); // Compile and execute the expression
        ...
        /// </summary>
        /// <param name="orderCreateDto">Order creation data transfer object</param>
        /// <param name="userId">Authenticated user ID</param>
        /// <returns>Tuple containing success status, error message (if any), and payment URL</returns>
            ...
            using var transaction = await _context.Database.BeginTransactionAsync();
            {
                ...
                try
                {
                    ...
                    var paymentRequest = new PaymentRequest
                    {
                        ...
                        CallBackUrl = "https://alshamiaa.com/ar/cart?success_pay=true",
                        ErrorUrl = "https://alshamiaa.com/ar/cart?success_pay=false"
                    };
        ...
        /// <param name="request">Status update request DTO</param>
        /// <returns>Tuple containing success status, error message (if any), and updated order data</returns>
        ...
        public async Task<(bool isSuccess, ErrorResponse? errorMessage, PagedList<GetOrderDataResponse>? result)>
      GetAllOrdersAsync(string? status, string? userName, int page, string? userId = null)
        {
