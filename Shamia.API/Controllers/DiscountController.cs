...
namespace Shamia.API.Controllers
{
    public class DiscountController : ControllerBase
    {
        ...
        // POST: api/discount/{id}
        [HttpPost("api/discount/{id}")]
        public async Task<IActionResult> UpdateDiscount(int id, [FromBody] JsonElement jsonData)
        {
            try
            {
                var updatedDiscount = JsonSerializer.Deserialize<Discount>(jsonData.ToString(), new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                });

                if (updatedDiscount == null || id != updatedDiscount.DiscountId)
                {
                    return BadRequest("Invalid request data.");
                }

                if (jsonData.TryGetProperty("Start_Date", out JsonElement startDateElement) && startDateElement.ValueKind == JsonValueKind.String)
                {
                    if (DateTime.TryParse(startDateElement.GetString(), out DateTime parsedStartDate))
                    {
                        updatedDiscount.StarDate = parsedStartDate;
                    }
                }

                if (jsonData.TryGetProperty("End_Date", out JsonElement endDateElement) && endDateElement.ValueKind == JsonValueKind.String)
                {
                    if (DateTime.TryParse(endDateElement.GetString(), out DateTime parsedEndDate))
                    {
                        updatedDiscount.EndDate = parsedEndDate;
                    }
                }
