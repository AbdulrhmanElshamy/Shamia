...
{
    /// <summary>
    /// Provides dashboard analytics and data export functionality
    /// </summary>
    public class DashboardService : IDashboardService
    {
        private readonly ShamiaDbContext _context;

        public DashboardService(ShamiaDbContext context)
        {
            _context = context;
        }
        ...
        /// <returns>Tuple containing success status, error message (if any), and statistics data</returns>
        ...
        /// <summary>
        /// Exports order data to Excel format with detailed product information
        /// </summary>
        /// <returns>Tuple containing success status, error message (if any), and Excel file bytes</returns>
        ...
        private async Task<int> GetFilteredUserCountAsync(DateTime? startDate, DateTime? endDate)
        {
            ...
            if (startDate.HasValue && endDate.HasValue)
                query = query.Where(u =>
                    ...
                    (u.Created_At == DateTime.MinValue && 
                    new DateTime(2025, 1, 1) >= startDate && 
                    new DateTime(2025, 1, 1) <= endDate)
                );
